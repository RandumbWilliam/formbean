...
<template>
  ...
  <section v-else-if="status === 'success' && data">
        ...
        <div class="flex gap-3">
          <span class="flex items-center gap-2">
            <span
              ...
              :class="data.published ? 'bg-green-500' : 'bg-gray-400'"
            />
            <span class="text-xs">
          ...
          <Button type="button" @click.prevent="onPublish">
            Publish
          </Button>
        </div>
      </template>
    ...
    <FormRender v-if="preview" :form="data.draftForm" :submitted @submit="onSubmit" />
